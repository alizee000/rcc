...
export default function InvitePlayersModal({ meetupId, currentParticipants, onClose }: { meetupId: string, currentParticipants: string[], onClose: () => void }) {
  const [searchQuery, setSearchQuery] = useState("");
  const users = useQuery(api.users.getUsers) || [];
  const inviteToMeetup = useMutation(api.meetups.inviteToMeetup);
  const router = useRouter();

  const handleInvite = async (userId: string) => {
    try {
      await inviteToMeetup({ meetupId: meetupId as Id<"meetups">, userId });
      router.refresh();
      onClose(); // Auto close for simplicity, or we could leave it open to invite more
    } catch (e) {
      console.error(e);
      ...
      <div style={{
        ...
        <div style={{ padding: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12, overflowY: "auto", maxHeight: "40vh" }}>
            {users
              .filter(u => u.clerkId && u.name.toLowerCase().includes(searchQuery.toLowerCase()))
              .map(user => {
                const isAlreadyParticipant = currentParticipants.includes(user.clerkId);
                return (
                  <div key={user._id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    {isAlreadyParticipant ? (
                      <div style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 600 }}>Invited/Joined</div>
                    ) : (
                      <button 
